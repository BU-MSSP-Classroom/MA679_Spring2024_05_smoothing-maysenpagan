...
```{r,echo=TRUE}
set.seed(123)
inds <- sample(seq_len(nrow(College)), size = nrow(College)*(0.75))
train <- College[inds,]
test <- College[-inds,]

model <- lm(log(Outstate) ~ Private + Top10perc + Expend + log(Room.Board), data = train)

preds1 <- predict(model, newdata = test)
sum((preds1-log(test$Outstate))^2)
```
...
```{r,echo=TRUE}
model2 <- gam(log(Outstate) ~ Private + bs(Top10perc) + poly(Expend, 2) + s(log(`Room.Board`)), data = train)
...
sum((preds2-log(test$Outstate))^2)
```
...
```{r}
set.seed(123)
inds <- sample(seq_len(nrow(Auto)), size = nrow(Auto)*(0.75))
train <- Auto[inds,]
test <- Auto[-inds,]
```
...
model <- gam(mpg ~ poly(horsepower,2) + bs(weight) + bs(displacement):bs(acceleration), data = train)
```

```{r}
preds <- predict(model, newdata = test)
sum((preds-test$mpg)^2)
